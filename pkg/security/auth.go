...
func FormatUserScopes(certUserScope []CertificateUserScope) string {
	...
	for _, scope := range certUserScope {
		...
		if scope.Global {
			...
			fmt.Fprintf(&buf, "tenant %v", scope.TenantIdentity)
		}
		comma = ", "
	}
	return buf.String()
...
func ValidateUserScope(
	...
	for _, scope := range certUserScope {
		if scope.Username == user {
			// If username matches, allow authentication to succeed if the
			// tenant identity is a match or if the certificate scope is
			// global.
			if scope.Global {
				return true
			}
