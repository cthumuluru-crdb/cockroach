...
func UserAuthCertHook(
	...
	return func(ctx context.Context, systemIdentity username.SQLUsername, clientConnection bool) error {
		...
		if ValidateUserScope(certUserScope, systemIdentity.Normalized(), tenantID, tenantName, roleSubject, certSubject) {
			if certManager != nil {
...
func ValidateUserScope(
	...
	for _, scope := range certUserScope {
		if scope.Username == user {
			...
			if scope.Global || scope.TenantID == tenantID || scope.TenantName == tenantName {
				return true
			}
		}
	}
