...
func (s *topLevelServer) newTenantServer(
	...
	baseCfg, sqlCfg, err := s.makeSharedProcessTenantConfig(ctx, tenantID, tenantNameContainer.Get(), portStartHint, tenantStopper, testArgs.Settings)
	if err != nil {
		return nil, err
	}
...
func (s *topLevelServer) makeSharedProcessTenantConfig(
	...
	baseCfg, sqlCfg, err := makeSharedProcessTenantServerConfig(ctx, tenantID, tenantName, portStartHint, parentCfg, localServerInfo, st, stopper, s.recorder)
	if err != nil {
