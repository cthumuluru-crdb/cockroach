...
func (d *nodeClientDialer) DialKeyVisualizerClient(
...
) (keyvispb.RPCKeyVisualizerClient, error) {
	...
	return keyvispb.NewGRPCKeyVisualizerClientAdapter(conn), nil
}

func (d *nodeClientDialer) DialMultiRaftClient(
	ctx context.Context, nodeID roachpb.NodeID, class rpc.ConnectionClass,
) (kvserver.RPCMultiRaftClient, error) {
	nd := (*nodedialer.Dialer)(d)
	conn, err := nd.Dial(ctx, nodeID, class)
	if err != nil {
		return nil, err
	...
	return kvserver.NewGRPCMultiRaftClientAdapter(conn), nil
}
