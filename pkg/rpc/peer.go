...
func (p *peer[Conn]) runOnce(ctx context.Context, report func(error)) error {
	...
	if err := runSingleHeartbeat(
		ctx,
		p.connOptions.newHeartbeatClient(cc),
		p.k,
...
func (p *peer[Conn]) runHeartbeatUntilFailure(
	...
	// use.
	heartbeatClient := p.connOptions.newHeartbeatClient(p.snap().c.connFuture.Conn())
...
func (p *peer[Conn]) onQuiesce(report func(error)) {
	...
	var nilConn Conn
	p.snap().c.connFuture.Resolve(nilConn, errQuiescing)
}
