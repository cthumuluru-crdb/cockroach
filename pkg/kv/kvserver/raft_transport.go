...
func (t *RaftTransport) processQueue(
	q *raftSendQueue, stream RPCMultiRaft_RaftMessageBatchClient, _ rpc.ConnectionClass,
) error {
	errCh := make(chan error, 1)
...
func (t *RaftTransport) startProcessNewQueue(
	...
	worker := func(ctx context.Context) {
		...
		client, err := AsClientDialer(t.dialer).DialMultiRaftClient(ctx, toNodeID, class)
		if err != nil {
			return
		}
...
func (t *RaftTransport) SendSnapshot(
	...
	client, err := AsClientDialer(t.dialer).DialMultiRaftClient(ctx, nodeID, rpc.DefaultClass)
	if err != nil {
		return nil, err
	}
