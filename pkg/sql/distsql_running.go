...
func (req runnerRequest) run() error {
	...
	client, err := AsClientDialer(req.sqlInstanceDialer).DialDistSQLClient(
		req.ctx, roachpb.NodeID(req.sqlInstanceID), rpc.DefaultClass,
	)
	if err != nil {
		...
		_ = dsp.stopper.RunAsyncTask(initCtx, "distsql-canceling-worker", func(parentCtx context.Context) {
			...
			for {
				...
				case <-dsp.cancelFlowsCoordinator.workerWait:
					...
					client, err := AsClientDialer(dsp.sqlInstanceDialer).DialDistSQLClient(
						parentCtx, roachpb.NodeID(sqlInstanceID), rpc.DefaultClass)
					if err != nil {
