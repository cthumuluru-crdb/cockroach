...
type TenantIdentifier interface {
	...
	// String returns the string representation of the tenant identifier.
	String() string

	// Bytes returns byte representation the the tenant identifier.
	Bytes() []byte

	// Returns true if this tenant identifier is equal to the other tenant
	// identifier.
	IsEqual(TenantIdentifier) bool
}
...
func (this TenantID) IsEqual(that TenantIdentifier) bool {
...
func (tname TenantName) String() string {
	return string(tname)
}

func (this TenantName) IsEqual(that TenantIdentifier) bool {
	that1, ok := that.(TenantName)
	if !ok {
		return false
	}
	return this.Equal(that1)
}
